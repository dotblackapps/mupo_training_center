...
<header class="mupo-dashboard-topbar">
    ...
    <div class="mupo-topbar-actions">
        ...
        <details class="mupo-profile-menu"><summary><span class="mupo-profile-avatar">@if(!empty($user->image))<img src="{{ getProfileImage($user->image, $user->name) }}" alt="{{ $user->name }}">@else{{ strtoupper(substr($user->name, 0, 1)) }}{{ strtoupper(substr(strrchr(' '.$user->name, ' '), 1, 1)) }}@endif</span><span class="mupo-profile-copy"><strong>{{ $user->name }}</strong><small>Learner</small></span><i class="fas fa-chevron-down"></i></summary>
            <div class="mupo-profile-dropdown"><a href="{{ $profileUrl }}"><i class="far fa-user"></i><span>My Profile</span></a><a href="{{ route('users.settings') }}"><i class="fas fa-cog"></i><span>Account Settings</span></a><div class="mupo-profile-divider"></div><a class="mupo-logout-link" href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('mupo-dashboard-logout-form').submit();"><i class="fas fa-sign-out-alt"></i><span>Log Out</span></a><form id="mupo-dashboard-logout-form" action="{{ route('logout') }}" method="POST" class="d-none">@csrf</form></div>
        </details>
    </div>
</header>
